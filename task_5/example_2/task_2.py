...
def big_query_3(collection):
    q = [
        {
            "$match" : {
                'city' : {'$in' : ['Алма-Ата', 'Сеговия', 'Хельсинки', 'Фигерас']},
                'job' : {'$in' :['Врач', 'Медсестра', 'Водитель']},
                '$or' : [
                    {'salary': {'$gt' : 90000, '$lt' : 170000}}]
                }},
        {
            '$group' : {
                "_id" : 'result',
    ...
    for stat in collection.aggregate(q):
        items.append(stat)
    write_result_to_json(items, 'big_query_3.json')
